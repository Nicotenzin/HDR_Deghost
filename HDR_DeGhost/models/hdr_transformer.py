...
def window_partition(x, window_size):
    # (B,H,W,C) 变为 (B*H*W//window_size//window_size, window_size, window_size, C)
    B, H, W, C = x.shape
...
def window_reverse(windows, window_size, H, W):
    # (B*H*W//window_size//window_size,window_size,window_size,C)变回 (B,H,W,C)
...
class PatchEmbed(nn.Module):
    ...
    Intput:
        x: (B, C, H, W)
    Return:
        x: (B, num_patch, embed_dim)
    """
    def __init__(self, img_size, patch_size, in_chans=6, embed_dim=96, norm_layer=None):
        super().__init__()
...
class HDRTransformer(nn.Module):
    ...
    def forward_features(self, x):
        # x [B, embed_dim, h, w] (4, 64, 256, 256)
        x_size = (x.shape[2], x.shape[3])
        x = self.patch_embed(x)  # B L C  (4,
        if self.ape:
