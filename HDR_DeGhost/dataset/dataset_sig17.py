...
class SIG17_Training_Dataset(Dataset):
    # root_dir = ./data  sub_set=sig17_training_crop256_stride64
    def __init__(self, root_dir, sub_set, is_training=True):
        ...
        self.scenes_dir = osp.join(self.root_dir, self.sub_set)
        # self.scenes_dir = ./data/sig17_training_crop256_stride64
        self.scenes_list = sorted(os.listdir(self.scenes_dir)) # os.listdir()返回指定文件夹下包含的文件列表
...
class SIG17_Validation_Dataset(Dataset):
    ...
    def __init__(self, root_dir, is_training=False, crop=True, crop_size=256):
        self.root_dir = root_dir
        self.is_training = is_training
        self.crop = crop
